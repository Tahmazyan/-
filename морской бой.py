        ...
        for ship_size in Game.ships_rules:
            ...
            while True:
                ...
                if player.auto_ship_setup is not True:
                    ...
                    print('{}. Расстановка кораблей...'.format(player.name))
    ...
    def get_input(self, input_type):
        ...
        if input_type == "ship_setup":
            ...
            if x not in Game.letters or not y.isdigit() or int(y) not in range(1, Game.field_size + 1) or \
                    r not in ("H", "V"):
                self.message.append('Приказ не выполнен, ошибка ввода координат')
                return 0, 0, 0
...
if __name__ == '__main__':
    ...
    while True:
        ...
        if game.status == 'in game':
            ...
            Game.clear_screen()
            game.current_player.message.append("К выстрелу готов!: ")
            game.draw()
            ...
            if shot_result == 'miss':
                game.next_player.message.append('Цельтесь точнее {}, мимо! '.format(game.current_player.name))
                game.next_player.message.append('Ходит {}!'.format(game.next_player.name))
                game.switch_players()
                continue
            elif shot_result == 'retry':
                game.current_player.message.append('Дополнительный выстрел!')
                continue
            elif shot_result == 'get':
                game.current_player.message.append('Точный выстрел, продолжайте!')
                game.next_player.message.append('Наш корабль под обстрелом!')
                continue
            elif shot_result == 'kill':
                game.current_player.message.append('Корабль противника потоплен!')
                game.next_player.message.append('Плохие новости, наш корабль тонет! :(')
                continue
        ...
        if game.status == 'game over':
            ...
            print('{} выиграл! Поздравления!'.format(game.current_player.name))
            break
